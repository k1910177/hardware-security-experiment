...
void invTransform(unsigned char *state, unsigned char *key, int num) {
...
void Transform(unsigned char *state, unsigned char *key, int num) {
    unsigned char w[Nk * Nb * (Nr + 1)];
    ...
    // Transform
    for (int i = 0; i < num; i++) {
        AddRoundKey(state, state, w + i * 16);
...
int main() {
    ...
    printf("* PLAIN TEXT *\n");
    invTransform(ST1, TrueKey, 2);
    invTransform(ST2, TrueKey, 2);
